...
export async function POST(request: Request) {
  ...
  try {
    ...
    // SECURITY: Validate destination address format for the specified target blockchain network
    if (!isValidAddress(destAddress, destChain)) {
      logger.warn({ destAddress, destChain }, 'Invalid destination address for chain');
      return NextResponse.json({ error: 'Invalid destination address for chain' }, { status: 400 });
    }
