...
export async function POST(request: Request) {
  ...
  try {
    ...
    // SECURITY: Validate destination address format against target blockchain to prevent loss of funds or upstream API failures
    if (!isValidAddress(destAddress, destChain)) {
      logger.warn({ destAddress, destChain }, 'Invalid destination address format for target chain');
      return NextResponse.json({ error: 'Invalid destination address for specified chain' }, { status: 400 });
    }
