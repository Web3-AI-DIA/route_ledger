...
export async function POST(request: Request) {
  ...
  try {
    ...
    // SECURITY: Validate destination address format for target chain to prevent loss of funds and invalid external API requests.
    if (!isValidAddress(destAddress, destChain)) {
      logger.warn({ destAddress, destChain }, 'Invalid destination address for target chain');
      return NextResponse.json({ error: 'Invalid destination address for target chain' }, { status: 400 });
    }
