 ...
 * Redacts PII (email, displayName, photoUrl) before logging to prevent leakage.
 */
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null) {
  const errInfo: FirestoreErrorInfo = {
    ...
    authInfo: {
      ...
      // Redact PII to prevent leakage to logs and third-party services
  ...
  // Use structured logging with redacted metadata
  logger.error({ err: error, metadata: errInfo }, 'Firestore Error');

  // Throw a generic error message to prevent leaking internal metadata to the UI
