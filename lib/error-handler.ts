...
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null) {
  // SECURITY: Redact PII (email, displayName, photoUrl) from metadata before logging
  ...
  // SECURITY: Use centralized logger with structured metadata
  logger.error({ err: error, metadata: errInfo }, 'Firestore operation failed');

  // SECURITY: Return a generic error message to prevent leaking internal state or PII to the client UI
