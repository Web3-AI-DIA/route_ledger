...
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null) {
  ...
  // SECURITY: Use centralized logger and redact PII to prevent leakage
  logger.error({ err: error, metadata: errInfo }, 'Firestore operation failed');

  // SECURITY: Throw a generic error message to the client to prevent internal detail leakage
